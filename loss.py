...
def adversarial_loss(discriminator, real_img=None, fake_img=None, target_label=1, y_org=None, y_trg=None):
    """
    ...
    if real_img is not None and y_org is not None:
        real_label = torch.full_like(discriminator(real_img, y_org), fill_value=target_label)
    ...
    if fake_img is not None and y_trg is not None:
        fake_label = torch.full_like(discriminator(fake_img, y_trg), fill_value=(1 - target_label))
...
def loss_generator(nets, x_real, y_org, y_trg, z_trgs=None, x_refs=None, lambda_sty=1.0, lambda_ds=1.0, lambda_cyc=1.0):
    ...
    # Compute losses
    loss_adv = adversarial_loss(nets.discriminator, fake_img=x_fake, target_label=1, y_trg=y_trg)
...
def loss_discriminator(nets, x_real, y_org, y_trg, z_trg=None, x_ref=None, lambda_reg=1.0):
    ...
    loss_real = adversarial_loss(nets.discriminator, real_img=x_real, target_label=1, y_org=y_org)
